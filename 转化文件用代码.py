...
# 类别映射
classes = ["liner", "bulk carrier", "warship","sailboat","canoe","container ship","fishing boat"]

# 数据路径
data_dir = ''
images_dir = 'images'
labels_dir = 'labels/trainxml'
...
def convert_annotation(xml_file, output_txt_path):
    ...
    try:
        with open(output_txt_path, 'w') as f:
            for obj in root.iter('object'):
                ...
                try:
                    ...
                    # 关闭已打开的XML文件
                    tree = None  # 解除ET的对XML文件的占用
                    # 删除原始XML文件
                    os.remove(xml_file)
                    file_deleted = True  # 将"文件已删除"标志设置为True
                    raise  # 继续向上层传播异常
        ...
        if file_deleted:  # 如果文件已被删除
            os.remove(output_txt_path)  # 删除文件
